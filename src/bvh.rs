...
impl BvhNode {
    pub fn new(objects: &mut [Primitive], start_index: usize) -> Self {
        ...
        if span == 1 {
            return BvhNode::Leaf { bbox: objects[0].bounding_box(), object_offset: start_index }
        }
        ...
        objects.sort_by(comparator); // mutable sort
        ...
        BvhNode::Interior { bbox: bbox_union, left: Box::new(left), right: Box::new(right) }
    } 
}
...
impl Hittable for BvhNode {
    ...
    fn bounding_box(&self) -> Aabb {
        match self {
            BvhNode::Leaf {bbox, object_offset: _ } => *bbox,
            BvhNode::Interior { bbox, left: _, right:_ } => *bbox, 
        }
    }
}